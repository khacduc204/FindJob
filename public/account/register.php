...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formValues['role'] = $_POST['role'] ?? '3';
    
    $password = $_POST['password'] ?? '';
    $result = $auth->register($formValues['email'], $password, $formValues['name'] ?: null, (int)$formValues['role']);
    
    if ($result['success']) {
        // Auto login after register
        $login = $auth->login($formValues['email'], $password);
        if ($login['success']) {
        ...
        header('Location: /JobFind/public/account/login.php');
        exit;
    }
    $error = $result['message'] ?? 'Không thể đăng ký. Vui lòng thử lại.';
}
?>
<!DOCTYPE html>
<html lang="vi">
                ...
                <div class="row g-0">
                    <div class="col-lg-6">
                        <div class="auth-form-pane">
                            ...
                            <div class="mb-4">
                                <h2 class="fw-bold mb-1">Tạo tài khoản JobFind</h2>
                            ...
                            <form method="post" class="auth-form">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-lg">
                                        ...
                                        <input type="email" name="email" value="<?= htmlspecialchars($formValues['email']) ?>" class="form-control" placeholder="Email" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-lg">
                                        ...
                                        <input type="text" name="name" value="<?= htmlspecialchars($formValues['name']) ?>" class="form-control" placeholder="Ví dụ: Nguyễn Văn A">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-lg">
                                        ...
                                        <input type="password" name="password" class="form-control js-password" placeholder="Tối thiểu 6 ký tự" required>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="role" class="form-select form-select-lg">
                                        <option value="3" <?= $formValues['role'] == '3' ? 'selected' : '' ?>>Ứng viên tìm việc</option>
                                        <option value="2" <?= $formValues['role'] == '2' ? 'selected' : '' ?>>Nhà tuyển dụng</option>
                                    </select>
                                </div>
                            ...
                            <p class="mt-4 text-center text-muted">Đã có tài khoản? <a href="/JobFind/public/account/login.php" class="auth-link">Đăng nhập ngay</a></p>
